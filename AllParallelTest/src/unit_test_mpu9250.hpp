...
void mpu9250_loop()
{
  ...
  if (!AHRS)
  {
    ...
    if (myIMU.delt_t > 500)
    {
      if (SerialDebug)
      {
        ...
        Serial.printf("acceleration(mg): X-%f Y-%f Z-%f \n", 1000 * myIMU.ax, 1000 * myIMU.ay, 1000 * myIMU.az);
        
        // Print gyro values in degree/sec!
        Serial.printf("gyro rate(degrees/sec): X-%f Y-%f Z-%f \n", myIMU.gx, myIMU.gy, myIMU.gz);
        
        // Print mag values in degree/sec
        Serial.printf("mag field(mG): X-%f Y-%f Z-%f \n", myIMU.mx, myIMU.my, myIMU.mz);
        
        myIMU.tempCount = myIMU.readTempData(); // Read the adc values
        // Temperature in degrees Centigrade
        myIMU.temperature = ((float)myIMU.tempCount) / 333.87 + 21.0;
        // Print temperature in degrees Centigrade
        Serial.printf("Temperature is(mG): %f degrees C \n", myIMU.temperature);
      }
  ...
  else
  {
    ...
    if (myIMU.delt_t > 500)
    {
      ...
      {
        // Print acceleration values in milligs!
        Serial.printf("a(mg): X-%f Y-%f Z-%f \n", 1000 * myIMU.ax, 1000 * myIMU.ay, 1000 * myIMU.az);
        
        // Print gyro values in degree/sec!
        Serial.printf("g(deg/s): X-%f Y-%f Z-%f \n", myIMU.gx, myIMU.gy, myIMU.gz);
        
        // Print mag values in degree/sec
        Serial.printf("m(mG): X-%f Y-%f Z-%f \n", myIMU.mx, myIMU.my, myIMU.mz);

        // Print mag values in degree/sec
        Serial.printf("q: q0-%f qx-%f qy-%f qz-%f \n", *getQ(), *(getQ() + 1), *(getQ() + 2), *(getQ() + 3));
      }
